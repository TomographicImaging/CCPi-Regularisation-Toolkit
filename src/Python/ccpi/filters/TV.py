...
def TV_ROF_CPU(inputData, regularisation_parameter, iterationsNumb, 
               marching_step_parameter, tolerance_param, out=None, infovector=None):
...
#       float epsil, int dimX, int dimY, int dimZ);
    cilreg.TV_ROF_CPU_main.argtypes = [ ctypes.POINTER(ctypes.c_float),  # pointer to the input array
...
#       float *lambdaPar, int lambda_is_arr, int iterationsNumb, float tau, 
#       float epsil, int dimX, int dimY, int dimZ);
    cilreg.TV_ROF_CPU_main(in_p, out_p, infovector_p, 
                ctypes.byref(ctypes.c_float(regularisation_parameter)), 
                0, 
                iterationsNumb, 
                marching_step_parameter, tolerance_param, 
                dims[0], dims[1], dims[2])

    return out
   
#%% From copilot chat
def TV_FGP_CPU(inputData, lambdaPar, iterationsNumb, epsil, methodTV, nonneg, out=None, infovector=None):
...
def SB_TV_CPU(inputData, mu, iter, epsil, methodTV, out=None, infovector=None):
    ...
    # float SB_TV_CPU_main(float *Input, float *Output, float *infovector, float mu, 
    # int iter, float epsil, int methodTV, int dimX, int dimY, int dimZ);
    cilreg.SB_TV_CPU_main(in_p, out_p, infovector_p, mu, iter, epsil, methodTV, dims[0], dims[1], dims[2])

    return out

def LLT_ROF_CPU(inputData, lambdaROF, lambdaLLT, iterationsNumb, tau, epsil, out=None, infovector=None):
    ...
    # float LLT_ROF_CPU_main(float *Input, float *Output, float *infovector, float lambdaROF, 
    # float lambdaLLT, int iterationsNumb, float tau, float epsil, int dimX, int dimY, int dimZ);
    cilreg.LLT_ROF_CPU_main(in_p, out_p, infovector_p, lambdaROF, lambdaLLT, iterationsNumb, tau, epsil, dims[0], dims[1], dims[2])

    return out

# TGV 
def TGV_CPU(inputData, lambdaPar, alpha1, alpha0, iterationsNumb, L2, epsil, out=None, infovector=None):
    ...
    # float TGV_main(float *Input, float *Output, float *infovector, float lambdaPar, 
    # float alpha1, float alpha0, int iterationsNumb, float L2, float epsil, int dimX, int dimY, int dimZ);
    cilreg.TGV_main(in_p, out_p, infovector_p, lambdaPar, alpha1, alpha0, iterationsNumb, L2, epsil, dims[0], dims[1], dims[2])

    return out

# dTV
def dTV_FGP_CPU(inputData, inputRef, lambdaPar, iterationsNumb, epsil, eta, methodTV, nonneg, out=None, infovector=None):
    ...
    # dTV_FGP_CPU_main(float *Input, float *InputRef, float *Output, float *infovector, float lambdaPar, 
    # int iterationsNumb, float epsil, float eta, int methodTV, int nonneg, int dimX, int dimY, int dimZ);
    cilreg.dTV_FGP_CPU_main(in_p, inref_p, out_p, infovector_p, lambdaPar, iterationsNumb, epsil, eta, methodTV, nonneg, dims[0], dims[1], dims[2])
...
def PatchSelect_CPU(inputData, H_i, H_j, H_k, Weights, SearchWindow, SimilarWin, NumNeighb, h, dims):
    ...
    if inputData.ndim != 2:
        # See https://github.com/vais-ral/CCPi-Regularisation-Toolkit/issues/184
...
def NLTV(inputData, H_i, H_j, H_k, Weights, NumNeighb, lambdaReg, IterNumb, 
                    switchM=1, Output=None):
    ...
    # https://github.com/vais-ral/CCPi-Regularisation-Toolkit/blob/71f8d304d804b54d378f0ed05539f01aaaf13758/src/Python/src/cpu_regularisers.pyx#L689
    ...
    dims = list(inputData.shape)
    if inputData.ndim != 2:
        raise ValueError(f'NLTV can only process 2D data. Got {inputData.ndim} dimensions')
...
if cilregcuda is not None:
    def TV_ROF_GPU(inputData, regularisation_parameter, iterationsNumb, 
        ...
        #                     int N, int M, int Z);
        cilregcuda.TV_ROF_GPU_main.argtypes = [ ctypes.POINTER(ctypes.c_float),  # pointer to the input array
                                ctypes.POINTER(ctypes.c_float),  # pointer to the output array
                                ctypes.POINTER(ctypes.c_float),  # pointer to the infoVector array
    ...
    #       float epsil, int dimX, int dimY, int dimZ);
        if gpu_device == 'gpu':
            gpu_device = 0
        cilregcuda.TV_ROF_GPU_main(in_p, out_p, infovector_p, 
    ...
    def TV_FGP_GPU(inputData, lambdaPar, iterationsNumb, epsil, methodTV, nonneg, gpu_device, 
                   out=None, infovector=None):
        ...
        # int iter, float epsil, int methodTV, int nonneg, int gpu_device, int N, int M, int Z);
        cilregcuda.TV_FGP_GPU_main(in_p, out_p, infovector_p, lambdaPar, iterationsNumb, epsil, methodTV, 
                                nonneg,
                                gpu_device, dims[0], dims[1], dims[2])

        return out
    
    def PDTV_GPU(Input, lambdaPar, iter, epsil, lipschitz_const, methodTV, nonneg, gpu_device, Output=None, infovector=None):
    ...
    def SB_TV_GPU(inputData, lambdaPar, iterationsNumb, epsil, methodTV, 
                       gpu_device, out=None, infovector=None):
        ...
        result = cilregcuda.TV_SB_GPU_main(in_p, out_p, infovector_p, lambdaPar, 
                                           iterationsNumb, epsil, methodTV, 
                                           gpu_device, dims[0], dims[1], dims[2])

        return out
    
    def LLT_ROF_GPU(inputData, lambdaROF, lambdaLLT, iterationsNumb, tau, epsil, gpu_device, out=None, infovector=None):
        ...
        # int LLT_ROF_GPU_main(float *Input, float *Output, float *infovector, float lambdaROF, float lambdaLLT, 
        # int iterationsNumb, float tau,  float epsil, int gpu_device, int N, int M, int Z);
        result = cilregcuda.LLT_ROF_GPU_main(in_p, out_p, infovector_p, 
                                    lambdaROF, lambdaLLT, iterationsNumb, tau, 
                                    epsil, gpu_device, dims[0], dims[1], dims[2])

        return out
    
    def TGV_GPU(inputData, lambdaPar, alpha1, alpha0, iterationsNumb, L2, epsil, 
                     gpu_device, out=None, infovector=None):
        ...
        # int TGV_GPU_main(float *Input, float *Output, float *infovector, float lambdaPar, float alpha1, float alpha0, 
        # int iterationsNumb, float L2, float epsil, int gpu_device, int dimX, int dimY, int dimZ);
        result = cilregcuda.TGV_GPU_main(in_p, out_p, infovector_p, lambdaPar, 
                                    alpha1, alpha0, iterationsNumb, L2, 
                                    epsil, gpu_device, dims[0], dims[1], dims[2])

        return out
    
    def dTV_FGP_GPU(inputData, inputRef, lambdaPar, iterationsNumb, epsil, eta, 
                         methodTV, nonneg, gpu_device, out=None, infovector=None):
        ...
        # int dTV_FGP_GPU_main(float *Input, float *InputRef, float *Output, float *infovector, float lambdaPar, 
        # int iterationsNumb, float epsil, float eta, int methodTV, int nonneg, int gpu_device, int N, int M, int Z);
        result = cilregcuda.dTV_FGP_GPU_main(in_p, inref_p, out_p, infovector_p, lambdaPar, iterationsNumb, epsil, eta, 
                                    methodTV, nonneg, gpu_device, dims[0], dims[1], dims[2])
