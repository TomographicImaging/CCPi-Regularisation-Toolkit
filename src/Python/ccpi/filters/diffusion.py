...
def NDF_CPU(inputData, lambdaPar, sigmaPar, iterationsNumb, tau, penaltytype, epsil, out=None, infovector=None):
    ...
    # float Diffusion_CPU_main(float *Input, float *Output, float *infovector, float lambdaPar, 
    # float sigmaPar, int iterationsNumb, float tau, int penaltytype, float epsil, int dimX, int dimY, int dimZ);
    cilreg.Diffusion_CPU_main(in_p, out_p, infovector_p, lambdaPar, sigmaPar, iterationsNumb, tau,
                              penaltytype, epsil, dims[0], dims[1], dims[2])

    return out

def Diffus4th_CPU(inputData, lambdaPar, sigmaPar, iterationsNumb, tau, epsil, out=None, infovector=None):
    ...
    # float Diffus4th_CPU_main(float *Input, float *Output,  float *infovector, float lambdaPar, 
    # float sigmaPar, int iterationsNumb, float tau, float epsil, int dimX, int dimY, int dimZ);
    cilreg.Diffus4th_CPU_main(in_p, out_p, infovector_p, lambdaPar, sigmaPar, iterationsNumb, tau, epsil, dims[0], dims[1], dims[2])

    return out


if cilregcuda is not None:
    def NDF_GPU(inputData, lambdaPar, sigmaPar, iterationsNumb, tau, 
                      penaltytype, epsil, gpu_device, out=None, infovector=None):
        ...
        # int NonlDiff_GPU_main(float *Input, float *Output, float *infovector, float lambdaPar, float sigmaPar, 
        # int iterationsNumb, float tau, int penaltytype, float epsil, int gpu_device, int N, int M, int Z);
        result = cilregcuda.NonlDiff_GPU_main(in_p, out_p, infovector_p, lambdaPar, 
                                            sigmaPar, iterationsNumb, tau,
                                            penaltytype, epsil, gpu_device,
                                            dims[0], dims[1], dims[2])

        return out
       
    def Diffus4th_GPU(inputData, lambdaPar, sigmaPar, iterationsNumb, tau, epsil, gpu_device, out=None, infovector=None):
        ...
        # int Diffus4th_GPU_main(float *Input, float *Output, float *infovector, float lambdaPar, float sigmaPar, 
        # int iterationsNumb, float tau, float epsil, int gpu_device, int N, int M, int Z);
        result = cilregcuda.Diffus4th_GPU_main(in_p, out_p, infovector_p, lambdaPar, 
                                            sigmaPar, iterationsNumb, tau, 
                                            epsil, gpu_device, 
                                            dims[0], dims[1], dims[2])
